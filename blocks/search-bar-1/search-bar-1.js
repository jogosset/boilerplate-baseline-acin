...
const SEARCH_SCOPE_PREFIX = 'search-bar-1-block';
...
function normalizeViewAllMode(value, fallback = DEFAULT_VIEW_ALL_MODE) {
  ...
  console.warn(`search-bar-1: invalid searchbar-viewall "${value}". Using "${fallback}".`);
  return fallback;
}
...
function normalizePanelMaxHeight(value, fallback = DEFAULT_PANEL_MAX_HEIGHT_PX) {
  ...
  if (!Number.isFinite(parsed)) {
    // eslint-disable-next-line no-console
    console.warn(`search-bar-1: invalid searchbar-maxheight "${value}". Using "${fallback}".`);
    return fallback;
  }
...
export default async function decorate(block) {
  ...
  searchBarContainer.classList.add('search-bar-1-container', `search-bar-1--${config.position}`);
  searchBarContainer.setAttribute('aria-expanded', 'false');
  searchBarContainer.dataset.searchStatus = 'initializing';

  const form = document.createElement('form');
  form.classList.add('search-bar-1-form');
  ...
  searchIconButton.classList.add('search-bar-1-icon');
  ...
  inputWrapper.classList.add('search-bar-1-input-wrapper');
  ...
  resultsDiv.classList.add('search-bar-1-results');
  ...
  liveRegion.classList.add('search-bar-1-sr-only');
  liveRegion.setAttribute('role', 'status');
  liveRegion.setAttribute('aria-live', 'polite');
  liveRegion.setAttribute('aria-atomic', 'true');

  const fallbackNote = document.createElement('p');
  fallbackNote.classList.add('search-bar-1-fallback-note');
  ...
  try {
    ...
    render.render(SearchResults, {
      ...
      slots: {
        Footer: async (ctx) => {
          ...
          viewAllResultsWrapper.classList.add('search-bar-1-view-all');
    ...
    // eslint-disable-next-line no-console
    console.error('search-bar-1: inline search unavailable. Falling back to submit-only mode.', error);
