...
The `search-bar-1` block is a standalone storefront search input with inline product results, powered by product discovery drop-ins.
...
Create a 1-row, 1-column `search-bar-1` block.
...
Place **Section Metadata immediately above** the `search-bar-1` block.
