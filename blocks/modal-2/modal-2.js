...
export default async function createModal(contentNodes) {
  await loadCSS(`${window.hlx.codeBasePath}/blocks/modal-2/modal-2.css`);
  const dialog = document.createElement('dialog');
  dialog.setAttribute('tabindex', 1);
  dialog.setAttribute('role', 'dialog');

  const dialogContent = document.createElement('div');
  dialogContent.classList.add('modal-2-content');
  ...
  closeButton.setAttribute('data-dismiss', 'modal-2');
  ...
  const closeModal = () => {
    // close the dialog
    dialog.close();
    // unmount any dropin containers rendered in the modal-2
  ...
  const block = buildBlock('modal-2', '');
  document.querySelector('main').append(block);

  dialog.addEventListener('close', () => {
    document.body.classList.remove('modal-2-open');
    block.remove();
  });
  ...
  return {
    ...
    showModal: () => {
      ...
      document.body.classList.add('modal-2-open');
    },
  };
}
