...
function warnOnInvalidConfig(name, rawValue, normalizedValue, fallback) {
  ...
  if (raw !== normalized && normalized === fallbackValue) {
    console.warn(`hero-cta-1: invalid ${name} "${rawValue}". Using "${fallback}".`);
  }
}

function warnOnNoOpConfig(name, rawValue, reason) {
  if (!rawValue || !rawValue.toString().trim()) return;
  console.warn(`hero-cta-1: ${name} "${rawValue}" has no effect. ${reason}`);
}
...
function parseTypedRows(rows) {
  ...
  rows.forEach((row, index) => {
    ...
    if (!type) {
      console.warn(`hero-cta-1: row ${rowNum} missing row type. Expected "slide", "nav", or "interval".`);
      return;
    }
    ...
    if (type === 'interval') {
      ...
      if (!Number.isNaN(ms) && ms > 0) {
        ...
        console.warn(`hero-cta-1: row ${rowNum} interval "${raw}" is invalid. Using "${DEFAULT_INTERVAL}".`);
      }
      return;
    }
    ...
    if (type === 'nav') {
      ...
      if (!label) {
        console.warn(`hero-cta-1: row ${rowNum} nav entry has no label.`);
        return;
      }
    ...
    console.warn(`hero-cta-1: row ${rowNum} has unsupported type "${type}".`);
  });
...
function buildSidebar(navEntries) {
  ...
  nav.className = 'hero-cta-1-sidebar';
  nav.setAttribute('aria-label', 'Hero navigation');

  const list = document.createElement('ul');
  list.className = 'hero-cta-1-sidebar-list';

  navEntries.forEach((entry) => {
    const li = document.createElement('li');
    li.className = 'hero-cta-1-sidebar-item';

    if (entry.type === 'header') {
      const header = document.createElement('div');
      header.className = 'hero-cta-1-sidebar-header';
      header.append(cloneLabelContent(entry.labelCell, entry.label));
      li.append(header);
    } else {
      const link = document.createElement('a');
      link.className = 'hero-cta-1-sidebar-link';
...
function extractImageSource(cell) {
  if (!cell) {
    console.warn('hero-cta-1: No image cell found');
    return null;
  }
  ...
  console.warn('hero-cta-1: No valid image source found in image cell.');
  return null;
}
...
function buildSlide(row, isFirstSlide = false, config = {}) {
  ...
  slide.className = 'hero-cta-1-slide';
  ...
  if (imageData) {
    ...
    media.className = 'hero-cta-1-media';
  ...
  content.className = 'hero-cta-1-content';
  ...
  if (buttonGroups.length > 0) {
    ...
    actionsWrapper.className = 'hero-cta-1-actions';
  ...
  overlay.className = 'hero-cta-1-overlay';
...
export default function decorate(block) {
  ...
  wrapper.className = 'hero-cta-1-slides';
  ...
  if (sidebarPosition && navEntries.length > 0) {
    ...
    layout.className = 'hero-cta-1-layout';
  ...
  if (layoutConfig.sidebarPosition && layoutConfig.layoutWidthRaw === 'full-width') {
    console.warn('hero-cta-1: herocta-width "full-width" is ignored when herocta-sidebar is enabled. Using "default".');
  }
  ...
  const maxButtonsPerSlide = Math.max(
    0,
    ...[...wrapper.querySelectorAll('.hero-cta-1-slide')]
      .map((slide) => slide.querySelectorAll('.hero-cta-1-actions .button').length),
  );
  ...
  block.style.setProperty('--hero-cta-1-content-max-width', `${layoutConfig.contentMaxWidth}px`);

  block.style.setProperty('--hero-cta-1-button-bg', resolveSurfaceColor(colorConfig.buttonFillColor));
  block.style.setProperty('--hero-cta-1-button-border', resolveSurfaceColor(colorConfig.buttonBorderColor));
  block.style.setProperty('--hero-cta-1-button-text-color', resolveButtonTextColor(colorConfig.buttonTextColor));
  block.style.setProperty('--hero-cta-1-button-border-width', `${styleConfig.buttonBorderWidth}px`);
  ...
  const slides = [...block.querySelectorAll('.hero-cta-1-slide')];
  ...
  const firstImage = block.querySelector('.hero-cta-1-media img');
  if (firstImage) {
