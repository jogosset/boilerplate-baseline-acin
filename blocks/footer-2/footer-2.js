...
/**
 * loads and decorates the footer-2
 * @param {Element} block The footer-2 block element
 */
export default async function decorate(block) {
  const root = getRootPath();
  // Load Footer as Fragment
  const footer-2Meta = getMetadata('footer-2');
  const footer-2Path = footer-2Meta ? new URL(footer-2Meta, window.location).pathname : '/footer-2';
  const fragment = await loadFragment(footer-2Path);

  // decorate footer-2 DOM
  block.textContent = '';
  const footer-2 = document.createElement('div');
  ...
  if (isMultistore()) {
    ...
    const $storeSwitcherBtn = footer-2.querySelector(
  ...
  while (fragment.firstElementChild) footer-2.append(fragment.firstElementChild);

  block.append(footer-2);
}
