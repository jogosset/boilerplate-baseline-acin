...
function warnOnInvalidConfig(name, rawValue, normalizedValue, fallback) {
  ...
  if (raw !== normalized && normalized === fallbackValue) {
    console.warn(`promotional-hero-2: invalid ${name} "${rawValue}". Using "${fallback}".`);
  }
}

function warnOnNoOpConfig(name, rawValue, reason) {
  if (!rawValue || !rawValue.toString().trim()) return;
  console.warn(`promotional-hero-2: ${name} "${rawValue}" has no effect. ${reason}`);
}
...
function createDescription(cell, config = {}) {
  ...
  wrapper.className = 'promotional-hero-2-description';
  ...
  if (config.descriptionMaxLines && config.descriptionMaxLines !== 'none') {
    wrapper.style.setProperty('--promotional-hero-2-description-max-lines', config.descriptionMaxLines);
  }
...
function buildCard(row, index, config) {
  ...
  card.className = 'promotional-hero-2-card';

  const imageData = extractImageSource(imageCell);
  if (imageData?.src) {
    const media = document.createElement('div');
    media.className = 'promotional-hero-2-image';
    ...
    console.warn('promotional-hero-2: row is missing a valid image source.');
  }

  const content = document.createElement('div');
  content.className = 'promotional-hero-2-whitebox';
  ...
  button.className = 'promotional-hero-2-button';
  ...
  if (safeHref) {
    ...
    if (buttonData.href && buttonData.href !== '#') {
      console.warn(`promotional-hero-2: blocked unsafe CTA URL "${buttonData.href}".`);
    }
  }
...
export default function decorate(block) {
  ...
  const applyResponsiveLayout = () => {
    ...
    block.style.setProperty('--promotional-hero-2-linear-max-cols', `${linearColumnCount}`);
    block.style.setProperty('--promotional-hero-2-layout-scale', layoutScale);
  };
  ...
  block.style.setProperty('--promotional-hero-2-button-bg', buttonFillColorToken);
  block.style.setProperty('--promotional-hero-2-button-border', buttonBorderColorToken);
  block.style.setProperty('--promotional-hero-2-button-text', buttonTextColorToken);
  block.style.setProperty('--promotional-hero-2-button-hover-bg', buttonHoverBgColorToken);
  block.style.setProperty('--promotional-hero-2-button-hover-border', buttonHoverBorderColorToken);
  block.style.setProperty('--promotional-hero-2-button-hover-text', buttonTextColorToken);
  block.style.setProperty('--promotional-hero-2-content-bg', contentBgColorToken);
  block.style.setProperty('--promotional-hero-2-content-color', contentTextColorToken);
  block.style.setProperty('--promotional-hero-2-content-border-color', contentBorderColorToken);
  block.style.setProperty('--promotional-hero-2-card-border-color', cardBorderColorToken);
  block.style.setProperty('--promotional-hero-2-image-overlay-color', imageOverlayColorToken);
  block.style.setProperty('--promotional-hero-2-card-border-width', `${styleConfig.cardBorderWidth}px`);
  block.style.setProperty('--promotional-hero-2-button-border-width', `${styleConfig.buttonBorderWidth}px`);
  block.style.setProperty('--promotional-hero-2-button-font-weight', styleConfig.buttonFontWeight);
  block.style.setProperty('--promotional-hero-2-background', backgroundColorToken);
  applyResponsiveLayout();

  const container = document.createElement('div');
  container.className = 'promotional-hero-2-container';
  ...
  const firstImage = block.querySelector('.promotional-hero-2-image img');
  if (firstImage) {
