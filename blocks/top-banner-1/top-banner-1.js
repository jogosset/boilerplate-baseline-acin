...
function debugLog(config, message, details = {}) {
  ...
  console.log(`top-banner-1: ${message}`, details);
}

function warnInvalidConfig(key, rawValue, fallback) {
  if (!rawValue || !rawValue.toString().trim()) return;
  // eslint-disable-next-line no-console
  console.warn(`top-banner-1: invalid ${key} "${rawValue}". Using "${fallback}".`);
}

function warnNoOpConfig(key, value, reason) {
  if (!value || !value.toString().trim()) return;
  // eslint-disable-next-line no-console
  console.warn(`top-banner-1: ${key} "${value}" has no effect. ${reason}`);
}
...
function sanitizeLinks(container) {
  ...
  links.forEach((link) => {
    ...
    if (!isSafeHref(href)) {
      // eslint-disable-next-line no-console
      console.warn(`top-banner-1: blocked unsafe href "${href}"`);
...
function warnIfLowContrast(block) {
  ...
  if (ratio !== null && ratio < 4.5) {
    // eslint-disable-next-line no-console
    console.warn(`top-banner-1: potential low contrast (${ratio.toFixed(2)}:1).`);
  }
}

function emitBlockEvent(block, name, detail = {}) {
  block.dispatchEvent(new CustomEvent(`top-banner-1:${name}`, {
    bubbles: true,
    detail,
  }));
}
...
function getHeaderMountedVisibleBanners() {
  return [...document.querySelectorAll('.top-banner-1[data-topbanner-mount="header"]')]
...
function applyPortableHeaderOffsets(totalHeight, gapToken) {
  ...
  if (safeHeight <= 0) {
    document.documentElement.style.removeProperty('--top-banner-1-height');
  ...
  document.documentElement.style.setProperty('--top-banner-1-height', `${safeHeight}px`);
...
function createTickerTrack(messages) {
  ...
  track.className = 'top-banner-1-ticker-track';

  messages.forEach((message, index) => {
    const item = document.createElement('span');
    item.className = 'top-banner-1-ticker-item';
    item.append(cloneCellContent(message));
    track.append(item);

    if (index < messages.length - 1) {
      const separator = document.createElement('span');
      separator.className = 'top-banner-1-ticker-separator';
...
function buildTicker(leftLane, rows, config, signal) {
  ...
  ticker.className = 'top-banner-1-ticker';
  ...
  const syncLayout = () => {
    ...
    if (tickerWidth > 0 && trackWidth > 0) {
      if (config.direction === 'right') {
        track.style.setProperty('--top-banner-1-track-start', `-${trackWidth}px`);
        track.style.setProperty('--top-banner-1-track-end', `${tickerWidth}px`);
      } else {
        track.style.setProperty('--top-banner-1-track-start', `${tickerWidth}px`);
        track.style.setProperty('--top-banner-1-track-end', `-${trackWidth}px`);
      }
    }
...
function appendLaneFragments(lane, cells) {
  ...
  visibleCells.forEach((cell, index) => {
    ...
    fragment.className = 'top-banner-1-fragment';
    fragment.append(cloneCellContent(cell));
    lane.append(fragment);

    if (index < visibleCells.length - 1) {
      const separator = document.createElement('span');
      separator.className = 'top-banner-1-fragment-separator';
      separator.textContent = '\u2022';
...
function runAutomatedChecks(block, config) {
  ...
  block.dispatchEvent(new CustomEvent('top-banner-1:checks', {
    bubbles: true,
    detail: {
      checks,
  ...
  if (checks.some((item) => !item.pass)) {
    // eslint-disable-next-line no-console
    console.warn('top-banner-1: automated checks found issues.', checks);
  } else {
    // eslint-disable-next-line no-console
    console.info('top-banner-1: automated checks passed.', checks);
  }
}
...
export default function decorate(block) {
  ...
  if (config.mount === 'header') {
    ...
    if (!mounted) {
      // eslint-disable-next-line no-console
      console.warn(`top-banner-1: header mount target not found ("${config.mountTarget}").`);
    }
  }

  block.style.setProperty('--top-banner-1-bg', resolveColorToken(config.bgColor));
  block.style.setProperty('--top-banner-1-text', resolveColorToken(config.textColor));
  ...
  inner.className = 'top-banner-1-inner';

  const leftLane = document.createElement('div');
  leftLane.className = 'top-banner-1-lane top-banner-1-lane-left';
  ...
  if (config.mode !== 'ticker' && effectiveLayout === 'multi') {
    ...
    if (centerCells.some(hasMeaningfulContent)) {
      ...
      centerLane.className = 'top-banner-1-lane top-banner-1-lane-center';
  ...
  if (config.mode !== 'ticker' && ['split', 'multi'].includes(effectiveLayout)) {
    ...
    if (rightCells.some(hasMeaningfulContent)) {
      ...
      rightLane.className = 'top-banner-1-lane top-banner-1-lane-right';
