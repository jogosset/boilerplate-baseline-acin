...
## Overview
`top-banner-1` renders a global announcement strip that can mount above the site header or render inline.
...
### Block Structure
`_top-banner-1.json` defines a `4`-column shape.
...
`_top-banner-1.json` currently defines no model fields.
...
Place `section-metadata` immediately above the section containing `top-banner-1`.
...
- Invalid values fall back safely and log `top-banner-1:` warnings.
...
- `top-banner-1:shown`
- `top-banner-1:ticker-start` (ticker mode only)
- `top-banner-1:checks` (debug mode only)
...
```js
(() => {
  const banner = document.querySelector('.top-banner-1');
  const ticker = banner?.querySelector('.top-banner-1-ticker');
  const track = ticker?.querySelector('.top-banner-1-ticker-track');
  console.table({
    ...
    items: ticker?.querySelectorAll('.top-banner-1-ticker-item').length ?? 0,
