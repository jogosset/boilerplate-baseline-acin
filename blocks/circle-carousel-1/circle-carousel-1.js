...
function updateCarousel(block, slideIndex) {
  const track = block.querySelector('.circle-carousel-1-track');
  const indicators = block.querySelectorAll('.circle-carousel-1-indicator');
  const items = block.querySelectorAll('.circle-carousel-1-item');
...
function createNavigation(block, totalItems) {
  ...
  nav.classList.add('circle-carousel-1-navigation');

  // Previous button
  const prevBtn = document.createElement('button');
  prevBtn.classList.add('circle-carousel-1-nav-btn', 'circle-carousel-1-prev');
  ...
  indicatorsContainer.classList.add('circle-carousel-1-indicators');
  ...
  for (let i = 0; i < totalSlides; i += 1) {
    ...
    indicator.classList.add('circle-carousel-1-indicator');
  ...
  nextBtn.classList.add('circle-carousel-1-nav-btn', 'circle-carousel-1-next');
...
/**
 * Decorates the circle-carousel-1 block
 * @param {Element} block - The circle-carousel-1 block element
 */
export default function decorate(block) {
  ...
  container.classList.add('circle-carousel-1-container');
  container.setAttribute('data-alignment', config.alignment);

  // Create viewport and track
  const viewport = document.createElement('div');
  viewport.classList.add('circle-carousel-1-viewport');

  const track = document.createElement('div');
  track.classList.add('circle-carousel-1-track');

  rows.forEach((row, index) => {
    const columns = [...row.children];

    const item = document.createElement('div');
    item.classList.add('circle-carousel-1-item');
    item.dataset.index = index;

    // Column 0: Circle image
    const circle = document.createElement('div');
    circle.classList.add('circle-carousel-1-circle');
    ...
    if (columns[1]) {
      ...
      textDiv.classList.add('circle-carousel-1-text');
