...
/**
 * loads and decorates the footer-3
 * @param {Element} block The footer-3 block element
 */
export default async function decorate(block) {
  const root = getRootPath();
  // Load Footer as Fragment
  const footer-3Meta = getMetadata('footer-3');
  const footer-3Path = footer-3Meta ? new URL(footer-3Meta, window.location).pathname : '/footer-3';
  const fragment = await loadFragment(footer-3Path);

  // decorate footer-3 DOM
  block.textContent = '';
  const footer-3 = document.createElement('div');
  ...
  if (isMultistore()) {
    ...
    const $storeSwitcherBtn = footer-3.querySelector(
  ...
  while (fragment.firstElementChild) footer-3.append(fragment.firstElementChild);

  block.append(footer-3);
}
