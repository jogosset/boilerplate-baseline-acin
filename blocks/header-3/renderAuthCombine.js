  ...
  if (accountLi) {
    ...
    authCombineLink.addEventListener('click', (event) => {
      ...
      function getPopupElements() {
        const header-3Block = document.querySelector('.header-3.block');
        const header-3LoginButton = document.querySelector('#header-3-login-button');
      ...
      events.on('authenticated', (isAuthenticated) => {
        ...
        if (isAuthenticated) {
          const { header-3LoginButton, popupElement, popupMenuContainer } = getPopupElements();
          ...
          if (header-3LoginButton) {
            const spanElementText = header-3LoginButton.querySelector('span');
