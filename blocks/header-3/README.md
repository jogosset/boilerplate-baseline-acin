...
No block configuration is read via `readBlockConfig()`. The header-3 uses metadata tags for fragment paths.

### URL Parameters

No URL parameters are directly read by the header-3 block.
...
The header-3 creates three main sections from the nav fragment:
...
- `header-3.js` - Main block logic, navigation setup, and tool integrations
- `header-3.css` - Styles for navigation, panels, and responsive layouts
