...
const BUTTON_COLORS = {
  ...
  inherit: {
    bg: 'color-mix(in srgb, var(--newsletter-2-text) 12%, transparent)',
    text: 'var(--newsletter-2-text)',
    border: 'color-mix(in srgb, var(--newsletter-2-text) 35%, transparent)',
  },
};
...
function setHeadingContent(target, cell, fallbackText) {
  ...
  if (hasRenderableContent(cell)) {
    ...
    if (paragraphs.length > 0) {
      paragraphs.forEach((paragraph) => {
        ...
        line.className = 'newsletter-2-heading-line';
  ...
  fallbackLine.className = 'newsletter-2-heading-line';
...
function applyButtonColor(block, state, color) {
  ...
  if (color.startsWith('#')) {
    ...
    block.style.setProperty(`--newsletter-2-button-${cssState}-bg`, color);
    block.style.setProperty(`--newsletter-2-button-${cssState}-text`, text);
    block.style.setProperty(`--newsletter-2-button-${cssState}-border`, color);
    return;
  }

  const named = BUTTON_COLORS[color] || BUTTON_COLORS.default;
  block.style.setProperty(`--newsletter-2-button-${cssState}-bg`, named.bg);
  block.style.setProperty(`--newsletter-2-button-${cssState}-text`, named.text);
  block.style.setProperty(`--newsletter-2-button-${cssState}-border`, named.border);
}
  ...
  const config = {
    align: normalizeAlign(
      getConfigValue(
        block.dataset.align,
        sectionData,
        ['newsletter-2Align', 'dataNewsletterAlign'],
        'center',
      ),
    ),
    buttonStyle: normalizeButtonStyle(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2ButtonStyle', 'dataNewsletterButtonStyle'],
        'default',
      ),
    ),
    buttonWidth: normalizeButtonWidth(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2ButtonWidth', 'dataNewsletterButtonWidth'],
        'medium',
      ),
    ),
    buttonColorEmpty: normalizeButtonColor(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2ButtonColorEmpty', 'dataNewsletterButtonColorEmpty'],
        'muted',
      ),
    ),
    buttonColorFilled: normalizeButtonColor(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2ButtonColorFilled', 'dataNewsletterButtonColorFilled'],
        'dark',
      ),
    ),
    backgroundColor: normalizeBackgroundColor(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2BackgroundColor', 'dataNewsletterBackgroundColor'],
        'neutral',
      ),
    ),
    backgroundStyle: normalizeBackgroundStyle(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2BackgroundStyle', 'dataNewsletterBackgroundStyle'],
        'solid',
      ),
    ),
    backgroundImage: getConfigValue(
      ...
      sectionData,
      ['newsletter-2BackgroundImage', 'dataNewsletterBackgroundImage'],
      '',
    ).trim(),
    emailStyle: normalizeEmailStyle(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2EmailStyle', 'dataNewsletterEmailStyle'],
        'outlined',
      ),
    ),
    density: normalizeDensity(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2Density', 'dataNewsletterDensity'],
        'comfortable',
      ),
    ),
    formGap: normalizeFormGap(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2FormGap', 'dataNewsletterFormGap'],
        'auto',
      ),
    ),
    fullWidth: normalizeBoolean(
      getConfigValue(
        ...
        sectionData,
        ['newsletter-2FullWidth', 'dataNewsletterFullWidth'],
        false,
      ),
      false,
    ),
...
export default function decorate(block) {
  ...
  const container = block.closest('.newsletter-2-container');
  ...
  wrapper.className = 'newsletter-2-inner';

  const title = document.createElement('h2');
  title.className = 'newsletter-2-heading';
  title.textContent = heading;

  const description = document.createElement('div');
  description.className = 'newsletter-2-subheading';

  const form = document.createElement('form');
  form.className = 'newsletter-2-form';
  form.method = 'post';
  form.noValidate = true;

  const fieldId = window.crypto?.randomUUID
    ? `newsletter-2-email-${window.crypto.randomUUID()}`
    : `newsletter-2-email-${Date.now()}`;
  const statusId = `${fieldId}-status`;
  const errorId = `${fieldId}-error`;

  const emailLabel = document.createElement('label');
  emailLabel.className = 'newsletter-2-sr-only';
  emailLabel.setAttribute('for', fieldId);
  emailLabel.textContent = 'Email address';

  const input = document.createElement('input');
  input.className = 'newsletter-2-email';
  input.id = fieldId;
  input.name = 'email';
  input.type = 'email';
  ...
  submitButton.className = 'newsletter-2-submit';
  submitButton.type = 'submit';
  submitButton.textContent = buttonLabel;

  const status = document.createElement('p');
  status.className = 'newsletter-2-status';
  status.id = statusId;
  status.setAttribute('role', 'status');
  status.setAttribute('aria-live', 'polite');

  const validationErrorEl = document.createElement('p');
  validationErrorEl.className = 'newsletter-2-error';
  ...
  supporting.className = 'newsletter-2-supporting';

  const success = document.createElement('div');
  success.className = 'newsletter-2-success';
  ...
  successTitle.className = 'newsletter-2-success-heading';
  successTitle.textContent = successHeading;

  const successBody = document.createElement('div');
  successBody.className = 'newsletter-2-success-subheading';

  const successLegal = document.createElement('div');
  successLegal.className = 'newsletter-2-success-supporting';
  ...
  if (config.backgroundColor.startsWith('#')) {
    ...
    block.style.setProperty('--newsletter-2-custom-bg', config.backgroundColor);
    block.style.setProperty('--newsletter-2-custom-text', getCustomTextColor(config.backgroundColor));
  }

  if (config.backgroundStyle === 'image-wash' && isSafeUrl(config.backgroundImage)) {
    block.style.setProperty('--newsletter-2-bg-image', `url("${config.backgroundImage}")`);
  }
  ...
  if (config.formGap.endsWith('px') || config.formGap.endsWith('rem') || config.formGap.endsWith('em')) {
    ...
    block.style.setProperty('--newsletter-2-form-gap', config.formGap);
  }
