...
export default async function decorate(block) {
  ...
  const sidebarItems = Array.from(sidebarItemsConfig).map((item) => {
    ...
    menuItemEl.classList.add('commerce-account-sidebar-2-item');
    ...
    if (isItemActive) {
      menuItemEl.classList.add('commerce-account-sidebar-2-item-active');
    }
...
function createMenuItemIcon(iconSource) {
  ...
  iconEl.classList.add('commerce-account-sidebar-2-item-icon');
...
function createMenuItemContent(title, subtitle) {
  ...
  contentEl.classList.add('commerce-account-sidebar-2-item-content');

  const titleEl = document.createElement('p');
  titleEl.classList.add('commerce-account-sidebar-2-item-title');
  titleEl.innerText = title;

  const subtitleEl = document.createElement('p');
  subtitleEl.classList.add('commerce-account-sidebar-2-item-subtitle');
...
function createMenuItemArrow() {
  ...
  arrowEl.classList.add('commerce-account-sidebar-2-item-arrow');
  UI.render(Icon, {
    source: 'ChevronRight',
    size: 32,
  })(arrowEl);
