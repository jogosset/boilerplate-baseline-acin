...
function addAnimation() {
  window.addEventListener('scroll', () => {
    const header-1 = document.getElementsByClassName('header-1-nav-wrapper')[0];
    const scrollPosition = window.scrollY;
    const viewportWidth = window.innerWidth;

    if (viewportWidth > 900) {
      if (scrollPosition > 168) {
        header-1.classList.add('minimized');
      } else {
        header-1.classList.remove('minimized');
      }
    } else {
      header-1.classList.remove('minimized');
    }
...
/**
 * decorates the header-1, mainly the nav
 * @param {Element} block The header-1 block element
 */
export default async function decorate(block) {
  ...
  navTools.querySelector('.nav-search-button').addEventListener('click', async () => {
    ...
    document.querySelector('header-1 .nav-search-input').classList.toggle('hidden');
    toggleSearch();
  });
  ...
  navWrapper.className = 'header-1-nav-wrapper';
